...
const movingAmbulanceArrowIcon = (rotation) =>
  L.divIcon({
    className:
...
function getRouteArrows(routePoints) {
  ...
  const step = Math.max(
    8,
    Math.floor(routePoints.length / 10)
  );
...
export default function LiveMap({
  ...
  const [deviceAmbulanceId, setDeviceAmbulanceId] =
    useState(() => {
      try {
        return (
          ambulanceId ||
        ...
        return DEFAULT_AMBULANCE_ID;
      }
    });

  const [idSaved, setIdSaved] =
    useState(true);
  ...
  const [routeStatus, setRouteStatus] =
    useState(
      "Waiting for actual Ambulance GPS..."
    );
  ...
  // ===================================================
  // SAFE INITIAL MAP CENTER
  // ===================================================
  //
  // IMPORTANT FIX:
  // MapContainer must NEVER receive null/undefined
  // as center.
  //
  ...
  useEffect(() => {
    ...
    // Load all ambulances already online.
    fetch(
  ...
  useEffect(() => {
    ...
    async function getRoute() {
      try {
        // =================================================
        // IMPORTANT FIX:
        // ambulanceLocation null होने पर destructuring
        // नहीं करना है।
        // इससे "xe is not iterable" error आता था।
        // =================================================
        ...
        if (
          ...
          if (
            ...
            throw new Error(
              "OSRM returned invalid route geometry"
            );
          }
          ...
          if (
            ...
            throw new Error(
              "No valid route points returned"
            );
          }
  ...
  return (
    <section className="map-card">
      ...
      >
        ...
        >
          This device sends only its
          real GPS location to the EMMC
          backend. Use a different unique
          ID on every ambulance phone.
        </small>
      </div>
        ...
        policeDistance !==
          ...
          policeInRange ? (
            <div className="alert">
              ...
              <div>
                🚦 Action: Traffic
                Police can coordinate
                traffic clearance for
                the ambulance.
              </div>
